...
if [[ $retval == 0 ]]; then
    ...
    for file in $(find -name '*.root' -or -name '*.xml'); do
        echo "cp -pv ${file} /eos/user/x/xisu/WorkSpace/VH_AC_Analysis/flashgg_output/UL_dataset/UL_17/UL_17_data_test2"
        cp -pv $file /eos/user/x/xisu/WorkSpace/VH_AC_Analysis/flashgg_output/UL_dataset/UL_17/UL_17_data_test2
        if [[ $? != 0 ]]; then
            errors="$errors $file($?)"
        fi
    done
